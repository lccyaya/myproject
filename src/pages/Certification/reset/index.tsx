...
const Login: React.FC<LoginProps> = (props) => {
  ...
  const handleSubmit = async (values: SummitTypes) => {
    report({
      action: REPORT_ACTION.forget_password_continue,
      cate: REPORT_CATE.forget_password,
    });
    setSummitLoading(true);
    const result = await certificationService.resetPassword({ ...data, password: md5(values.password) });
    setSummitLoading(false);
    if (result.success) {
      message.success(intl.formatMessage({
        id: 'key_successfully_modified',
      }));
      onSummit();
    } else {
