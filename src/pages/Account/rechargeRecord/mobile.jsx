import TradeRecord from '@/components/TradeRecord/mobile';
...
import Loading from '@/components/base-loading/mobile';
  ...
  return (
    <Spin spinning={loading}>
      <InfiniteScroll
        ...
        hasMore={!noMore}
        endMessage={null}
        loader={data?.list?.length ? <Loading /> : null}
      >
        <div className={styles.content}>
          {!loading ? (
            <>{data?.list?.length ? <TradeRecord list={data.list} /> : <Empty />}</>
          ) : null}
        </div>
      </InfiniteScroll>
    </Spin>
